...
import { Skeleton, Space, Card, Image, Alert } from 'antd';
import axios from 'axios';
import './Group.css';

import {
    SettingOutlined,PlusCircleOutlined,QuestionCircleOutlined,
    EditOutlined,ShoppingTwoTone ,ShoppingCartOutlined,
    EllipsisOutlined,UploadOutlined,VideoCameraOutlined
    ...
    render(){
        ...
        const itemlist = items.length ? (
            items.filter(item => item.groupid === groupid).map(item => {
                    return(  
                        ...
                        <Card key={item.itemid} bordered={true} style={{ width: 325 }}
                        cover={
                            <Image
                            src={item.image}
                          ...
                          actions={[
                            <NavLink key="details" exact to={"/item/"+item.itemid }   > <QuestionCircleOutlined  /></NavLink>,
                            <div>{item.price}US$</div>,
                            <form key="add" target="paypal" action="https://www.paypal.com/cgi-bin/webscr" method="post">
                                <input type="hidden" name="cmd" value="_s-xclick"/>
                                <input type="hidden" name="hosted_button_id" value={item.paypalbuttonid}/>
                                <button className="hidenbutton" type="text"><PlusCircleOutlined  ></PlusCircleOutlined ></button>
        ...
        return(
            ...
            <div>
                ...
                {(groupid ==="battery") ? <Alert type="warning" message={message3} ></Alert> : ''} 
                {(groupid ==="groupbuy") ? <Alert type="warning" message={message2}></Alert> : ''} 
